...
@implementation HHShowBigImgViewController
...
-(void)dealloc{
    ...
    NSLog(@"HHShowBigImgViewController dealloc");
}
...
- (void)initNavBtns
{
    ...
    UIImage *normalImg = [UIImage imageNamed:@"btn_circle.png"];
    UIImage *selImg = [UIImage imageNamed:@"checkbox_pic.png"];
...
- (void)initCollectionView
{
    ...
    [_collectionView registerClass:[HHBigImageCell class] forCellWithReuseIdentifier:@"HHBigImageCell"];
...
- (void)initBottomView
{
    ...
    CGFloat btnOriWidth = GetMatchValue(GetLocalLanguageTextValue(HHPhotoBrowserOriginalText), 15, YES, 30);
    ...
    NSLog(@"self.selectIndex====>%ld",self.selectIndex);
    weakify(self);
    PHAsset *indexAsset = self.assets[self.selectIndex];
    [[PHCachingImageManager defaultManager] requestImageDataForAsset:indexAsset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
        strongify(weakSelf);
        strongSelf.labPhotosBytes.text = [NSString stringWithFormat:@"%@", [strongSelf transformDataLength:imageData.length]];
    ...
    _btnDone.frame = CGRectMake(kViewWidth - 82, 8, 70, 28);
    [_btnDone setTitle:GetLocalLanguageTextValue(HHPhotoBrowserDoneText) forState:UIControlStateNormal];
...
- (void)btnDone_Click:(UIButton *)btn
{
    if (self.arraySelectPhotos.count == 0) {
        ...
        if (![[HHPhotoTool sharePhotoTool] judgeAssetisInLocalAblum:asset]) {
            ShowToastLong(@"%@", GetLocalLanguageTextValue(HHPhotoBrowserLoadingText));
            return;
        }
...
{
    if (self.arraySelectPhotos.count > 0) {
        [_btnDone setTitle:[NSString stringWithFormat:@"%@ (%ld)", GetLocalLanguageTextValue(HHPhotoBrowserDoneText), self.arraySelectPhotos.count] forState:UIControlStateNormal];
    } else {
        [_btnDone setTitle:GetLocalLanguageTextValue(HHPhotoBrowserDoneText) forState:UIControlStateNormal];
    }
}
...
//            strongify(weakSelf);
//            strongSelf.labPhotosBytes.text = [NSString stringWithFormat:@"(共%@)", photosBytes];
//        }];
//    } else {
...
- (void)showNavBarAndBottomView
{
    ...
    frame.origin.y -= frame.size.height + kHomeIndicator;
...
{
    HHBigImageCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HHBigImageCell" forIndexPath:indexPath];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (scrollView == (UIScrollView *)_collectionView) {
        ...
        [[PHCachingImageManager defaultManager] requestImageDataForAsset:_assets[str.integerValue] options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
