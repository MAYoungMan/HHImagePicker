...
@implementation HHPhotoAblumList
...
static HHPhotoTool *sharePhotoTool = nil;
...
- (NSArray<HHPhotoAblumList *> *)getPhotoAblumList
{
    NSMutableArray<HHPhotoAblumList *> *photoAblumList = [NSMutableArray array];
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if(collection.assetCollectionSubtype != 202 && collection.assetCollectionSubtype < 212){
            ...
            if (assets.count > 0) {
                HHPhotoAblumList *ablum = [[HHPhotoAblumList alloc] init];
                NSLog(@"ablum.title = %@",collection.localizedTitle);
                ablum.title =  [self getCollectionTitle:collection];
...
- (NSString *)getCollectionTitle:(PHAssetCollection *)collection
{
    ...
    switch (collection.assetCollectionSubtype) {
        case PHAssetCollectionSubtypeSmartAlbumUserLibrary:
            title = GetLocalLanguageTextValue(HHPhotoBrowserCameraRoll);
            break;
        case PHAssetCollectionSubtypeSmartAlbumPanoramas:
            title = GetLocalLanguageTextValue(HHPhotoBrowserPanoramas);
            break;
        case PHAssetCollectionSubtypeSmartAlbumVideos:
            title = GetLocalLanguageTextValue(HHPhotoBrowserVideos);
            break;
        case PHAssetCollectionSubtypeSmartAlbumFavorites:
            title = GetLocalLanguageTextValue(HHPhotoBrowserFavorites);
            break;
        case PHAssetCollectionSubtypeSmartAlbumTimelapses:
            title = GetLocalLanguageTextValue(HHPhotoBrowserTimelapses);
            break;
        case PHAssetCollectionSubtypeSmartAlbumRecentlyAdded:
            title = GetLocalLanguageTextValue(HHPhotoBrowserRecentlyAdded);
            break;
        case PHAssetCollectionSubtypeSmartAlbumBursts:
            title = GetLocalLanguageTextValue(HHPhotoBrowserBursts);
            break;
        case PHAssetCollectionSubtypeSmartAlbumSlomoVideos:
            title = GetLocalLanguageTextValue(HHPhotoBrowserSlomoVideos);
            break;
        case PHAssetCollectionSubtypeSmartAlbumSelfPortraits:
            title = GetLocalLanguageTextValue(HHPhotoBrowserSelfPortraits);
            break;
        case PHAssetCollectionSubtypeSmartAlbumScreenshots:
            title = GetLocalLanguageTextValue(HHPhotoBrowserScreenshots);
            break;
        default:
            break;
    }
...
- (void)requestImageForAsset:(PHAsset *)asset size:(CGSize)size resizeMode:(PHImageRequestOptionsResizeMode)resizeMode completion:(void (^)(UIImage *, NSDictionary *))completion
{
    ...
    [[PHCachingImageManager defaultManager] requestImageForAsset:asset targetSize:size contentMode:PHImageContentModeAspectFit options:option resultHandler:^(UIImage * _Nullable image, NSDictionary * _Nullable info) {
        ...
        if (isCloud && !image){
            PHImageRequestOptions *optionCloud = [[PHImageRequestOptions alloc] init];
            optionCloud.networkAccessAllowed = YES;
            
            requestID = [[PHCachingImageManager defaultManager] requestImageDataForAsset:asset options:optionCloud resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *photosBytes))completion
{
    ...
    for (int i = 0; i < photos.count; i++) {
        HHSelectPhotoModel *model = photos[i];
